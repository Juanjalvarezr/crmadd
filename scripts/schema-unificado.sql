-- ============================================================
-- CRM DESEO DIGITAL — Schema unificado completo para Supabase
-- Idempotente: todas las tablas, vistas, indices, RLS y datos base
-- Ejecutar en el SQL Editor del proyecto CRM
-- ============================================================
...
CREATE TABLE IF NOT EXISTS reglas_negocio_ai (
  id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS prompts_ai (
  id TEXT PRIMARY KEY,
...
-- ============================================================
-- 2) TABLAS NUEVAS USADAS POR RUTAS/SERVICES
-- ============================================================
...
-- ============================================================
-- 3) ROW LEVEL SECURITY + POLÍTICAS idempotentes
-- ============================================================
...
BEGIN
  RETURN QUERY
  SELECT ca.id, ca.titulo, ca.contenido, ca.categoria,
         1 - (ca.embedding <=> query_embedding) AS similarity
  FROM conocimiento_agencia ca
  WHERE 1 - (ca.embedding <=> query_embedding) > match_threshold
...
CREATE OR REPLACE VIEW vista_proyectos_cliente AS
SELECT
  p.id, p.nombre, p.descripcion, p.cliente_id, p.cliente_nombre,
  p.servicios, p.oportunidad_id, p.estado, p.prioridad,
  p.fecha_inicio, p.fecha_fin, p.progreso, p.presupuesto,
  p.costo_actual, p.tareas, p.recursos, p.monto_pagado,
  p.onboarding_checklist, p.estado_pago, p.metodo_pago,
  p.fase_administrativa, p.plan_contenido, p.creado_en, p.actualizado_en,
...
SELECT
  t.id, t.titulo, t.descripcion, t.fecha, t.prioridad, t.estado,
  t.created_at, t.proyecto_id, t.oportunidad_id, t.cliente_id,
  t.responsable_id, c.nombre AS cliente_nombre,
  COALESCE(e.nombre, 'Sin asignar') AS responsable_nombre
FROM tareas t
...
SELECT
  ...
  COALESCE((SELECT SUM(presupuesto) FROM proyectos),0) AS presupuesto_total;
...
CREATE INDEX IF NOT EXISTS idx_interacciones_cliente_id ON interacciones(cliente_id);
CREATE INDEX IF NOT EXISTS idx_conocimiento_embedding ON conocimiento_agencia USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64);
CREATE INDEX IF NOT EXISTS idx_oportunidades_abiertas ON oportunidades(etapa) WHERE estado = 'Abierta';
CREATE INDEX IF NOT EXISTS idx_proyectos_activos ON proyectos(estado) WHERE estado IN ('planificacion','en_progreso','pausado');
CREATE INDEX IF NOT EXISTS idx_tareas_pendientes ON tareas(estado) WHERE estado = 'Pendiente';
CREATE INDEX IF NOT EXISTS idx_clientes_activos ON clientes(estado) WHERE estado = 'Activo';
