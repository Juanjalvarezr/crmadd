...
interface Props {
  dbStatus: any;
  loading: boolean;
  onSeed?: (...args: any[]) => void | Promise<void>;
}

export const ConfigTabDatos = ({ dbStatus, loading, onSeed }: Props) => {
  const handleAction = async () => {
    if (typeof onSeed === 'function') await onSeed();
  };
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      <Paper sx={{ p: { xs: 2, sm: 3 }, borderRadius: 2 }}>
        ...
        <Button variant="contained" disabled={loading} onClick={handleAction} sx={{ backgroundColor: "#e91e63", "&:hover": { backgroundColor: "#c2185b" } }}>
          Cargar Datos Reales
        </Button>
      </Paper>
    </Box>
