...
export const mapDBToProyecto = (p: any) => ({
  ...
  oportunidadId: p.oportunidad_id != null ? String(p.oportunidad_id) : '',
  ...
  cronograma: Array.isArray(p.cronograma)
    ? p.cronograma
    : Array.isArray((p.cronograma || {}).items)
      ? (p.cronograma || {}).items
      : [],
...
export const mapProyectoToDB = (p: any) => ({
  ...
  cliente_nombre: String(p.clienteNombre || ''),
  servicios: p.servicios,
  oportunidad_id: p.oportunidadId ? Number(p.oportunidadId) : null,
  estado: p.estado,
