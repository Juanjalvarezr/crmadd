        ...
        {selectedProyecto && (
          <>
            <DialogTitle>
              ...
              <Tabs value={activeProjectTab} onChange={(_, v) => setActiveProjectTab(v)} textColor="primary" indicatorColor="primary" variant="scrollable" scrollButtons="auto" size="small" sx={{ mt: 1 }}>
              ...
              {activeProjectTab === 0 ? (
                ...
                <Box sx={{ py: 1 }}>
                  <Typography variant="caption" sx={{ mb: 2, display: 'block', color: 'text.secondary' }}>
                    Vincula carpetas de Drive, Hojas de Cálculo o Calendarios para este proyecto.
                  </Typography>
                  ...
                  <Stack spacing={1.5} sx={{ mb: 3 }}>
                    <TextField 
                      size="small" 
                      fullWidth 
                      label="Nombre del recurso (Ej: Estrategia SEO)" 
                      value={nuevoRecursoNombre}
                      onChange={(e) => setNuevoRecursoNombre(e.target.value)}
                    />
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      <TextField 
                        size="small" 
                        fullWidth 
                        placeholder="Pegar enlace de Google Drive/Sheet/Calendar..." 
                        value={nuevoRecursoUrl}
                        onChange={(e) => setNuevoRecursoUrl(e.target.value)}
                      />
                    </Box>
                    <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center' }}>
                      <Button size="small" variant="outlined" onClick={() => handleAddRecurso('drive')} startIcon={<Folder size={14}/>}>Drive</Button>
                      <Button size="small" variant="outlined" onClick={() => handleAddRecurso('sheet')} startIcon={<Layout size={14}/>}>Sheet</Button>
                      <Button size="small" variant="outlined" onClick={() => handleAddRecurso('calendar')} startIcon={<Calendar size={14}/>}>Calendar</Button>
                      <Button size="small" variant="outlined" onClick={() => handleAddRecurso('doc')} startIcon={<FileText size={14}/>}>Doc</Button>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'center', mt: 1 }}>
                      <Button 
                        ...
                        onClick={() => {
                          setNuevoRecursoNombre("Documento Identidad Digital");
                          setNuevoRecursoUrl("https://docs.google.com/document/u/0/"); // Aquí puedes poner el link a tu carpeta de plantillas
                        }}
                        sx={{ 
                          fontSize: '0.65rem', 
                          bgcolor: '#daa520', 
                      ...
                      >
                        Cargar Acceso Identidad Digital
                      </Button>
                      <Button
                        size="small"
                        variant="outlined"
                        startIcon={<FileText size={14}/>}
                        onClick={() => selectedProyecto && handleExportProjectToCSV(selectedProyecto)}
                        sx={{
                          ml: 1,
                        }}
                      >
                  ...
                  <List dense>
                    {(selectedProyecto.recursos || []).length === 0 ? (
                      <Typography variant="body2" color="text.secondary" align="center" sx={{ py: 2 }}>
                        No hay enlaces vinculados.
                      </Typography>
                    ) : (
                      selectedProyecto.recursos.map((recurso) => (
                        <ListItem 
                          key={recurso.id} 
                          sx={{ 
                        ...
                        >
                          <ListItemIcon sx={{ minWidth: 40 }}>
                            {recurso.tipo === 'drive' ? <Folder size={18} color="#FBBC04"/> : 
                             recurso.tipo === 'sheet' ? <Layout size={18} color="#0F9D58"/> : 
                             recurso.tipo === 'calendar' ? <Calendar size={18} color="#4285F4"/> :
                             <ExternalLink size={18} color="#EA4335"/>}
                          </ListItemIcon>
                          <ListItemText 
                            primary={<Typography variant="body2" sx={{ fontWeight: 'bold' }}>{recurso.nombre}</Typography>}
                            secondary={<Typography variant="caption" noWrap sx={{ display: 'block', maxWidth: 200 }}>{recurso.url}</Typography>}
                          />
