...
import { VentasIcon } from "../components/StatCard";
import {
...
export default function Ventas() {
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      ...
      globalSnack.show("Error al mover tarjeta: " + err.message, "error");
    }
  };
  ...
  const handleGenerarPropuestaAI = async (opp: Oportunidad) => {
    ...
    try {
      ...
      globalSnack.show("Error al generar propuesta", "error");
    } finally {
  ...
  const handleSave = async () => {
    if (!formData.nombre || !formData.cliente_nombre || formData.cliente_nombre === "__otro__") {
      globalSnack.show("Debes asignar un cliente real a la oportunidad.", "error");
      return;
    }

    if (formData.valor <= 0) {
      globalSnack.show("Asigna un valor estimado para proyectar tus ingresos.", "warning");
    }

    if ((formData.etapa === "Negociación" || formData.etapa === "Cierre") && formData.valor <= 0) {
      globalSnack.show("Se requiere un valor mayor a 0 para esta etapa.", "error");
      return;
    }
    ...
    try {
      ...
      if (editingOpp) {
        ...
        globalSnack.show("Oportunidad actualizada ✓", "success");
      } else {
      ...
      globalSnack.show("Error al guardar: " + err.message, "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (opp: Oportunidad) => {
    ...
    try {
      ...
      globalSnack.show("Oportunidad eliminada", "success");
    } catch (err: any) {
      globalSnack.show("Error al eliminar: " + err.message, "error");
    }
  };
              ...
              <Grid item xs={6} md={7}>
                ...
                <Button 
                  ...
                  onClick={() => { // Usar el Snackbar global
                    ...
                    globalSnack.show("Propuesta copiada al portapapeles", "success");
                  }}
                >
